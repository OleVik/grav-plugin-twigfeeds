...
| `twig_feeds`      | List: ...                                                                                          | List: `source` `name` `start` `end` `mode` `cache_time` `request_options` `categories` `tags` | `source`: URL for a RSS or Atom feed<br>`name`: Custom title of feed<br>`start`: Item to start the results from<br>`end`: Item to end the results with<br>`mode`: 'default', 'direct' or 'raw'<br>`cache_time`: Time, in seconds, to wait before caching data again<br>`request_options`: Like above, but for this feed specifically.<br>`categories` and `tags`: List of metadata strings |
...
In `/tests/feeds.json` there's a simple array of objects in the form `[{"source": "URL", "mode": "MODE"}]`, which can be ran with PHPUnit to verify the validity of sources and structure of returned data. See environment-variables in `/phpunit.xml` for settings.
